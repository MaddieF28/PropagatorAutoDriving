...
def testing_fall_duration():
    ...
    building_shadow.add_content(supported(make_interval(54.9, 55.1), ['shadows']))
    baromter_height.add_content(supported(make_interval(0.3, 0.32), ['shadows']))
    barometer_shadow.add_content(supported(make_interval(0.36, 0.37), ['shadows']))
    print(building_height.content)

    fall_time = Cell()
    fall_duration(fall_time, building_height)
    fall_time.add_content(supported(make_interval(2.9, 3.3), ['lousy-fall-time'])) # Not included as it is less informative than the better fall time
    print(building_height.content)

    fall_time.add_content(supported(make_interval(2.9, 3.1), ['better-fall-time'])) # premise is a strict subset of the same premises)
    print(building_height.content)

    building_height.add_content(supported(45,["superindendent"]))
    print(building_height.content)
    print(baromter_height.content)
