...
from tms_step_graph import StepGraph
...
# --------------------------------------------------------------------------
# brake_fallback bug: in the original code, CONSTRAINT_CHECKS["brake_for_front_risk"]
# was always called as CONSTRAINT_CHECKS[c](predicted, facts) -- but
# satisfies_brake_for_front_risk's signature is (action, facts). That means it was
# comparing a dict to the integer 4 and always returning False: the brake_fallback
# tier has never fired, for any action, ever. This is a real, pre-existing bug,
# unrelated to the TMS migration.
#
# Left False (default): the migration is a pure refactor with zero behavior change --
#   brake_fallback stays silently dead, exactly as it always was.
# Set True: brake_fallback becomes live for the first time (a real behavior change,
#   not a migration artifact). Flip this deliberately and note it when reporting
#   results, since it will change action selection whenever front_risk is True.
# --------------------------------------------------------------------------
FIX_BRAKE_FALLBACK_BUG = False
...
# ============================================================================
# CONSTRAINT LOGIC -- untouched. This is where the research novelty lives.
# Do not modify CONSTRAINT_TIERS, SITUATION_CONSTRAINTS, or infer_constraints
# as part of this or any future plumbing migration.
# ============================================================================
...
CONSTRAINT_TIERS = [
    ...
    ("no_regression",  ["no_lane_regression"]),
...
# ============================================================================
# REAL PROPAGATOR / TMS FACT GRAPH
# Replaces the old dict-mutation PROPAGATORS list + predict_effect dict-copy.
# One StepGraph is built once per real env step (see run()); all 5 candidate
# actions are evaluated against that SAME graph via graph.predict(action),
# using Hypothetical premises + bring_in/kick_out under the hood instead of
# rebuilding anything. See tms_step_graph.py for the mechanism itself.
# ============================================================================
...
def run_propagators(facts):
    """Kept for compatibility with call sites that just want facts derived
    once (e.g. the actual/no-candidate baseline for this step), not a
    counterfactual. Builds a throwaway one-candidate graph under the hood."""
    initialize_scheduler()
    graph = StepGraph(dict(facts), [1])  # action 1 = no-op, gives the "actual" derived facts
    derived = graph.predict(1)
    for key in ("ttc", "front_risk", "speed_slow", "speed_fast",
                "left_feasible", "right_feasible"):
        facts[key] = derived[key] if key in ("ttc", "front_risk", "speed_slow", "speed_fast") else facts.get(key)
    # left_feasible/right_feasible don't depend on the action, so compute them
    # directly here rather than through the graph (see StepGraph docstring).
    _compute_lateral_feasibility(facts)


def _compute_lateral_feasibility(facts):
    if facts.get("ego_lane") is not None and facts.get("num_lanes") is not None:
        if facts["ego_lane"] <= 0:
            facts["left_feasible"] = False
        elif facts["left_gap"] is None:
            facts["left_feasible"] = None
        else:
            facts["left_feasible"] = facts["left_gap"] >= .09

        if facts["ego_lane"] >= facts["num_lanes"] - 1:
            facts["right_feasible"] = False
        elif facts["right_gap"] is None:
            facts["right_feasible"] = None
        else:
            facts["right_feasible"] = facts["right_gap"] >= .09
    else:
        facts["left_feasible"] = None if facts["left_gap"] is None else facts["left_gap"] >= .09
        facts["right_feasible"] = None if facts["right_gap"] is None else facts["right_gap"] >= .09


def build_step_graph(facts):
    """Build the one persistent propagator graph for this real step."""
    initialize_scheduler()
    return StepGraph(dict(facts), ACTIONS)


def predict_effect(graph, action):
    """Drop-in replacement for the old predict_effect(action, facts) --
    same shape of return value, now backed by the TMS graph."""
    return graph.predict(action)


def evaluate_action(graph, action, facts, llm_action, intended):
    predicted = predict_effect(graph, action)
    scores = []
    results = {}
    for _, names in CONSTRAINT_TIERS:
        ...
        for c in names:
            ...
            elif c == "brake_for_front_risk":
                # See FIX_BRAKE_FALLBACK_BUG above: the original call site
                # passed `predicted` here, comparing a dict to an int and
                # always returning False. Preserved by default; flip the
                # flag to pass `action` and make this tier live.
                r = CONSTRAINT_CHECKS[c](action if FIX_BRAKE_FALLBACK_BUG else predicted, facts)
            else:
...
def select_action(facts, llm_action, situation=None):
    graph = build_step_graph(facts)

    intended = infer_constraints(llm_action, facts, situation)
    feasible = feasible_actions(graph, facts)
    ...
    llm_score, llm_results = evaluate_action(graph, llm_action, facts, llm_action, intended) \
    ...
    for action in sorted(feasible):
        score, results = evaluate_action(graph, action, facts, llm_action, intended)
        predicted = predict_effect(graph, action)
...
def update_facts_from_obs(obs, facts, mask_prob=0.0):
    obs = np.array(obs).flatten().reshape(5, 5)
    ego = obs[0]
    others = obs[1:]
...
def feasible_actions(graph, facts):
    feasible = {0, 1, 2, 3, 4}
...
def run(seed, mask_prob=0.0, mode="intent", target_rejection_rate=None, num_steps=3):
    ...
    override_reasons = {}
    tier_win_counts = np.zeros(len(CONSTRAINT_TIERS))
    infeasible_overrides = 0
    ...
    for _ in range(num_steps):
        ...
        if mode == "raw":
            ...
            graph = build_step_graph(facts)
            allowed = feasible_actions(graph, facts)
        ...
        graph_for_goal_check = build_step_graph(facts)
        predicted_for_chosen = predict_effect(graph_for_goal_check, action)
        ...
        if mode == "intent":
            if explanation["overridden"]:
                if not explanation["llm_feasible"]:
                    ...
                    graph_for_llm = build_step_graph(facts)
                    llm_score_vec, _ = evaluate_action(graph_for_llm, llmaction, facts, llmaction, intended_constraints)
...
if __name__ == "__main__":
    ...
    for s in seeds:
        (crash_rate, crashes, waypoints_reached, current_waypoint,
         final_distance_to_goal, route_progress, action_dist, speed,
         rejected_rate, rejected, known_frac, goal_abandoned,
         override_reasons, tier_win_counts, infeasible_overrides) = run(
